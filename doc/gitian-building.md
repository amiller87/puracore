...
*Setup instructions for a Gitian build of Pura Core using a Debian VM or physical system.*
...
- [Building Pura Core](#building-pura-core)
...
Clone the git repositories for Pura Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/purapay/pura
```
...
in the Pura Core repository under 'Fetch and build inputs' to install sources which require
...
To build Pura Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Pura Core repository.
...
and the pura git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../pura/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/pura.git
SIGPATH=/some/root/path/pura-detached-sigs.git

./bin/gbuild --url pura=${BTCPATH},signature=${SIGPATH} ../pura/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pura-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pura-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/pura-osx-build.assert
```
