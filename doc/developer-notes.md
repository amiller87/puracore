...
- ThreadSocketHandler : Sends/Receives data from peers on port 11001.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 11011 for connections and services them.
...
However, in open source software such as Pura Core, where everyone uses
...
pay attention to for reviewers of Pura Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Pura Core uses tinyformat, which is type safe. Leave them out to avoid confusion
